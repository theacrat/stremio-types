...
Generated with ts-rs, and cleaned up with [process-bindings.ts](./scripts/process-bindings.ts).
...
```ts
import type { MetaItem } from "stremio-core-types";
...
```ts
import { ExtraTypes, isExtraType } from "stremio-core-types";
